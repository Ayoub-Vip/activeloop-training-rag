...
class ExtractorAgent(Agent):
    def __init__(self, doc_type, doc_class):
        super().__init__()
        self.name = "document extractor agent"
        self.description="this agent extracts informations"
        self.llm = None
        self.prompt_tmplt_str = ExtractorPrompt().prompt_tmplt
        self.var_map_prompt = {
            "doc_type": doc_type,
            "doc_class": doc_class
        }
        self.prompt = RichPromptTemplate(
            template_str=self.prompt_tmplt_str,
            template_var_mappings=self.var_map_prompt
        )
        
        self.output_cls = get_output_cls(doc_class)
        
        self._agent = FunctionAgent(
            name=self.name,
            description=self.description,
            state_prompt=self.prompt,
